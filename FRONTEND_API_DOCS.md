...
## 8. User Account & Engagement APIs (New)

### 🔹 API: `POST /api/user/saved-colleges` [Auth Required]
* **Kaam**: Logged-in user ke liye college bookmark/save karna.
* **Body**: `{ "collegeId": "col_101" }`

### 🔹 API: `GET /api/user/saved-colleges` [Auth Required]
* **Kaam**: User ki sabhi saved colleges list fetch karna.

### 🔹 API: `DELETE /api/user/saved-colleges/:collegeId` [Auth Required]
* **Kaam**: Saved college list se college remove karna.

### 🔹 API: `GET /api/review/moderation-queue` [Admin Token]
* **Kaam**: Moderation ke liye pending reviews list.

### 🔹 API: `PUT /api/review/:id/approve` [Admin Token]
* **Kaam**: Review approve karna and college rating auto-update karna.

### 🔹 API: `PUT /api/review/:id/reject` [Admin Token]
* **Kaam**: Review reject karna with optional rejection reason.

### 🔹 API: `POST /api/activity/event`
* **Kaam**: User actions (search, view_college, view_course, compare, save, enquiry) log karna for analytics.
* **Body**: `{ "eventType": "view_college", "metadata": { "collegeId": "col_101" } }`

---

## 9. News, Articles & Content CMS APIs

### 🔹 API: `GET /api/blog`
* **Kaam**: Published articles, news, exam updates aur guides ki list fetch karna.
* **Params**: `status` (`PUBLISHED` default, `DRAFT`, `REVIEW`, `ALL`)
* **Response**:
```json
{
  "success": true,
  ...
  "data": [
    {
      "id": "blog_101",
      "title": "RUHS Nursing Entrance Exam 2026 Guide",
      "code": "ruhs-nursing-exam-2026",
      "contentType": "Exam Update",
      "description": "Full details on RUHS Nursing exam dates, syllabus and cutoffs.",
      "image": "ruhs_banner.jpg",
      "status": "PUBLISHED",
      "authorName": "Editor",
...
### 🔹 API: `GET /api/blog/:id`
* **Kaam**: Specific article/news piece ki complete body detail fetch karna.
